...
bool videoInput::getPixels( int id, unsigned char * dstBuffer, bool flipRedAndBlue, bool flipImage )
{
	...
	if( isDeviceSetup( id ) ) {
		if( bCallback ) {
			...
			if( success = VDList[id]->sgCallback->LockPixels( src ) ) {
				processPixels( src, dst, width, height, flipRedAndBlue, flipImage );
				VDList[id]->sgCallback->UnlockPixels();
			}
		}
		else {
...
bool videoInput::isFrameNew( int id )
{
	...
	bool hasNewFrame = VDList[id]->sgCallback->HasNewFrame();
	...
	if( VDList[id]->isFrozen && VDList[id]->autoReconnect ) {
